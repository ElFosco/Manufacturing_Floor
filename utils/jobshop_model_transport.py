...
class FJTransportProblem(FJProblem):
    ...
    def define_constraints(self):
        ...
        # conv_terms = {link: [] for link in self.y_conv.keys()}
        # all_transitions = self.get_all_transitions()
        # for i in self.I:
        #     for trans in all_transitions:
        #         for (m1, m2) in self.y_conv.keys():
        #             key = (i, trans, ("conv", m1, m2))
        #             if key in self.zT:
        #                 conv_terms[(m1, m2)].append(self.zT[key])
        #
        # # y_conv == OR(terms)  (use cp.max over booleans; if empty, fix to 0)
        # for link, lits in conv_terms.items():
        #     if lits:
        #         self.m += (self.y_conv[link] == sum(lits))
        #     else:
        #         self.m += (self.y_conv[link] == 0)
        ...
        self.resource_count = (sum(list(self.y_WS.values())) + sum(list(self.y_conv.values())) +
                               sum(list(self.y_h_use.values())) + sum(list(self.y_r_use.values())))
    ...
    def optimize(self,objective_type=0):
        ...
        if objective_type==0:
            ...
            last_E = [self.E[i, self.OPS_i[i][-1]] for i in self.I]
            self.m += (self.makespan == cp.max(last_E))
            self.m.minimize(int(1e6) * self.makespan + int(1e4)*self.resource_count + int(1e2)*self.humans_used  + self.robot_used)
            self.variables = {'makespan': self.makespan, 'resources': self.resource_count,
